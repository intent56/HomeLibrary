...
class FormFormat(FlaskForm):
    ...
    width = IntegerField("Высота", validators=[NumberRange(min=0, max=999)])
    comment = TextAreaField("Комментарий")
    submit = SubmitField("Добавить")
...
class FormBook(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.id_cover.choices = [(0, 'Не указан')] + [(c.id, f'{c.code} - {c.name}') for c in Cover.query.order_by('name').all()]
        self.id_format.choices = [(0, 'Не указан')] + [(f.id, f'{f.format}') for f in Format.query.order_by('format').all()]
        self.id_language.choices = [(0, 'Не указан')] + [(l.id, f'{l.code} - {l.name}') for l in Language.query.order_by('name').all()]
